...
*一级缓存，与Hibernate类似，提供以ID为主键的L1缓存，缓存在跨越多个方法的同一事务中有效。  
...
*没有对象关联、LazyLoad带来的OpenSessionInView问题和N+1问题, PO类可以直接传递到View层(JSP)，PO自带Dao类, 在View层事务不存在的情况下，依然可以继续存取数据库(通常只读)。  
*易学易维护易扩充，借用成熟工具(内部借用了Spring的JdbcTemplate，外部的事务可借用Spring的声明式事务)，此项目源码将不超过3000行。  
