...
###为什么要开发jSQLBox?
因为作者发现了一种利用Java来代替XML和Annotation配置的方法，凡是利用XML和Annotation作为配置文件的项目，都存在着XML不支持重构、Annotation不够灵活，配置文件不能动态生成、修改的问题，这对于需要动态生成或修改配置的场合是个致命缺陷。作者在完成jBeanBox项目后，发现Hibernate和MyBatiis这两个流行的持久层工具也都存在这个问题，这是jSqlBox项目产生的原因。简单说，jSqlBox的开发目标就是一个支持动态配置的Hibernate，笔者认为目前最好的配置方式不是XML,不是注解，不是JSON，不是模板，不是脚本，而是Java源代码本身，因为它功能强大而且支持重构。  

在开发之前，作者总结了一下Hibernate存在的一些问题,主要归纳如下:  
1)如前所述，配置是固定的，不能动态变化，对于需要在运行期动态创建或改变表名、列名、映射方式的场合，解决起来比较麻烦。  
2)缺省情况下，实体类为容器管理，导致任何对PO的更改都会写入数据库，这使得PO不能和VO合并，不能简单地传递到View层使用。  
3)HQL语言是重复发明轮子。另外HQL语言是虽然号称面向对象，但HQL语言本身不是面向对象的，不支持IDE拼写检查和重构。  
4)过度复杂，源码庞大(上万个类)。虽然号称对PO类无侵入，但实际上容器管理的ORM架构本身就是最大的侵入，采用了这种架构的项目就绑死在了Hibernate/JPA这种复杂的ORM工具上了。  
jSqlBox虽然最初目的是给Hibernate加一个动态配置，但考虑到容器管理实体ORM架构开发及使用的复杂性，以及个人水平有限，转向了类似MyBatis的架构，即在运行期手工创建SQL进行数据库读写，如须用到OR映射时，在程序中动态配置并完成OR转换。与MyBatis不同的是jSQLBox在易用性上作了极大改进，没有繁琐的XML配置或注解,无需手工创建CRUD的简单SQL。  
与目前流行的一些其它小众的持久层工具相比，jSQLBox则在功能上超出太多，例如一级(行级)缓存、二级缓存、动态配置、支持PO或SQL的重构等，很多持久层工具都不完全具备这些对性能和开发、维护至关重要的功能。
...
*低侵入，PO类只需要声明实现Entity接口即可(仅对Java8+,对于Java7及以下需要实做Entity接口方法，或继承自EntityBase类)。  
*无配置，默认情况下，PO类自动适应数据库表，字段自动匹配驼峰式或下划线式数据表列名，无需任何配置文件。  
*可配置，当数据库表名、字段名与缺省匹配规则不一致时，可用配置的方式来解决，配置为"同名+Box"的Java类，或直接写在初始化块中，这于jBeanBox项目的配置方式类似。  
*多配置和动态配置，同一个PO可采用不同的配置以进行多种方式的存取,配置可以继承重用已有的配置，配置可以动态生成和修改，这是XML和Annotation配置方式无法实现的。  
*(开发中)一级缓存与脏检查，与Hibernate类似，提供以ID为主键的行级缓存，一级缓存在跨越多个方法的同一事务中有效，对PO代理实例的存取不再重复访问数据库，除低数据库压力，与Hibernate的区别在于jSqlBox一级缓存只缓存实体，不缓存SQL命令，这是与全功能容器管理ORM工具的一个区别。  
...
*建立在E-R建模和手工SQL操作基础上，没有对象关联、懒加载带来的1+N或OpenSessionInView问题, PO类可以直接传递到View层(JSP)，PO自带Dao属性, 在View层事务不存在的情况下，依然可以继续存取数据库(通常只读)。  
