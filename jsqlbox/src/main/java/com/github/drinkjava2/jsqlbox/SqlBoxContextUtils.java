...
/**
 * This is utility class include static methods like CRUD methods belong to
 * SqlBoxContext
 * 
 * @author Yong Zhu
 * @since 1.0.0
...
public abstract class SqlBoxContextUtils {
	...
	public static int update(Object entityBean, SqlBox box) {
		...
		return box.context.nUpdate(sb.toString(), params.toArray(new Object[params.size()]));
	}
	...
	public static <T> T load(SqlBoxContext ctx, Class<?> entityClass, Object pkeyValue) {
		...
		SqlBox box = ctx.createSqlBox(entityClass);
		...
		if (pkeyColumns.size() != pkValueMap.size())
			throw new SqlBoxException("primary key column numbers doesn't match parameter numbers");
		...
		SqlBoxBindUtils.bindBoxToBean(box, entity, ctx);
		return (T) entity;
	}
