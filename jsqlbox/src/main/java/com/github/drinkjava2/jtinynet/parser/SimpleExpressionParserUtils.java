...
import com.github.drinkjava2.jtinynet.parser.SimpleExpressionParser.Item;

/**
 * public static methods for SimpleExpressionParser
 */
public class SimpleExpressionParserUtils {

	/** Execute function has 1 or 2 parameters */
	public static void doTheMath(Item func, Item last, Item next) {
		...
		if ("+".equals(f)) {
			if (lastType == '0' || lastType == 'F') {// only 1 parameter
				...
				if (lastType == 'S' || lastType == 'N' || lastType == 'B' || nextType == 'S' || nextType == 'N' || nextType == 'B') {
					func.type = 'S';
			...
			throw new TinyNetException("Un-parsed expression function: '" + f + "'");
	}

	private static void throwExp(String f) {
		throw new TinyNetException("Expression error near '" + f + "'");
	}
