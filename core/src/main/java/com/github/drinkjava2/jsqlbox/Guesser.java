...
public interface Guesser {// NOSONAR
	...
	public static class SimpleGuesser implements Guesser {
		...
		public <T> T guess(SqlBoxContext ctx, Object entity, Object... params) {// NOSONAR
			...
			if (callerMethod == null)
				throw new SqlBoxException(
						"Can not find method '" + callerMethodName + "' in '" + callerClassName + "'");
		...
		protected <T> T theGuessBody(SqlBoxContext ctx, Object entity, String callerClassName, Method callerMethod,
				Object... params) {
			PreparedSQL ps = GuesserUtils.getPreparedSqlAndHandles(callerClassName, callerMethod, ctx.getIocTool());
			...
			if (useTemplate)
				ps.setTemplateParamMap(GuesserUtils.buildParamMap(callerClassName, callerMethod.getName(), params));
			else
				ps.setParams(params);
			GuesserUtils.autoGuessHandler(entity, ps, sql, callerMethod); // guess handler
		...
		public String guessSQL(SqlBoxContext ctx, Object ac) {// NOSONAR
			...
			if (callerMethod == null)
				throw new SqlBoxException(
						"Can not find method '" + callerMethodName + "' in '" + callerClassName + "'");
			PreparedSQL sp = GuesserUtils.getPreparedSqlAndHandles(callerClassName, callerMethod, ctx.getIocTool());
			return sp.getSql();
		}
