...
public abstract class SqlBoxContextUtils {// NOSONAR
	...
	public static int insert(SqlBoxContext ctx, Object entityBean, Object... optionalSqlItems) {// NOSONAR
		...
		for (String fieldName : readMethods.keySet()) {
			...
			if (!col.getTransientable() && col.getInsertable()) {
				...
				if (col.getShardDatabase() != null) // Sharding database?
					sqlItems.append(shardDatabase(tableModel, ClassCacheUtils.readValueFromBeanField(entityBean, fieldName),
							null));
		...
		if (foundColumnToInsert)
			sqlItems.remove(sqlItems.size() - 1);// delete the last ", " character
	...
	public static <T> T load(SqlBoxContext ctx, Class<?> entityClass, Object pkeyValue, Object... optionalSqlItems) {// NOSONAR
		...
		for (String fieldName : writeMethods.keySet()) {
			...
			if (!col.getTransientable()) {
				...
				sqlItems.append(col.getColumnName()).append(", ");
				if (col.getPkey()) {
					pkeyColumns.add(col);
		...
		sqlItems.append(" from ").append(model.getTableName()).append(" where ");
		for (ColumnModel col : pkeyColumns)
			sqlItems.append(col.getColumnName()).append("=?").append(" and ");
		sqlItems.remove(sqlItems.size() - 1);// delete the last " and " character
		...
		try { 
			...
			sqlItems.append(SingleTonHandlers.arrayHandler);
			Object[] values = ctx.iQuery(sqlItems.toObjectArray());
