...
public abstract class SqlBoxContextUtils {// NOSONAR
	...
	/**
	 * Use current SqlBoxContext's shardingTools to calculate the real
	 * shardTable name
	 */
		...
		for (String fieldName : writeMethods.keySet()) {
			...
			if (!col.getTransientable()) {
				...
				if (col.getShardTable() != null) // Shard table?
					shardTableItem = shardTable(tableModel,
							ClassCacheUtils.readValueFromBeanField(tableModel, fieldName));
			}
		}
		jSQL.remove(jSQL.size() - 1);// delete the last ", "
		where.remove(jSQL.size() - 1);// delete the last " and"
		jSQL.insert(0, "select ");
