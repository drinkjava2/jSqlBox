...
public class InlineQueryRunner extends ImprovedQueryRunner {
	 ...
	 * ThreadLocal and return an empty String, a non parameter param0() call
	 * equal to clearAllInlineThreadlocalParams()
	 */
	...
	/**
	 * Clear all ThreadLocal parameters first, then cache parameters in
	 * ThreadLocal, then return a "?" String
	 */
	 ...
	 * "field1=?,field2=?.... fieldx=?" format inLineSQL String piece, and save
	 * bean property parameters in ThreadLocal
	 * 
	 * @param bean
	 ...
	 * @param inlineSQL
	 *            The in-line style SQL and parameters.
	 * @return The object returned by the handler.
	 * @throws SQLException
	 *             if a database access error occurs
	 */
	public <T> T inlineQuery(Connection conn, ResultSetHandler rsh, String... inlineSQL) throws SQLException {
	 ...
	 *             if a database access error occurs
	 */
	public <T> T inlineQuery(ResultSetHandler rsh, String... inlineSQL) throws SQLException {
		SqlAndParams sp = inlineToSqlAndParams(inlineSQL);
	...
	/**
	 * Executes the In-Line Style given INSERT, UPDATE, or DELETE SQL statement.
	 ...
	 *             if a database access error occurs
	 */
	public <T> T inlineInsert(ResultSetHandler rsh, String... inlineSQL) throws SQLException {
		SqlAndParams sp = inlineToSqlAndParams(inlineSQL);
	 ...
	 *             if a database access error occurs
	 */
	public <T> T inlineInsert(Connection conn, ResultSetHandler rsh, String... inlineSQL) throws SQLException {
		SqlAndParams sp = inlineToSqlAndParams(inlineSQL);
		return (T) insert(conn, sp.getSql(), rsh, sp.getParams());
	}

	/**
	 * Execute an In-Line style SQL statement, including a stored procedure
	 * call, which does not return any result sets. Any parameters which are
	 * instances of {@link OutParameter} will be registered as OUT parameters.
	 * <p>
	...
	/**
	 * Execute an In-Line style SQL statement, including a stored procedure
	 * call, which returns one or more result sets. Any parameters which are
	 * instances of {@link OutParameter} will be registered as OUT parameters.
	 * <p>
	 ...
	 * {@link #inlineExecute(java.sql.Connection, java.lang.String...) } (if
	 ...
	 *             if a database access error occurs
	 */
	public <T> List<T> inlineExecute(Connection conn, ResultSetHandler rsh, String... inlineSQL)
			throws SQLException {
		SqlAndParams sp = inlineToSqlAndParams(inlineSQL);
	 ...
	 * (if there are no OUT parameters) or
	 * {@link #inlineExecute(java.lang.String...) } (if there are no result
	 * sets).
	 *
	 * @param <T>
	 *            The type of object that the handler returns
	 ...
	 *             if a database access error occurs
	 */
	public <T> List<T> inlineExecute(ResultSetHandler  rsh, String... inlineSQL) throws SQLException {
		SqlAndParams sp = inlineToSqlAndParams(inlineSQL);
